...
- **create-release** — starts the next quarterly release: Release issue + `Bugs <version>` tech-epic in `ilm` with Project #5 fields, computed sprint schedule, and verification of the manual Sprint/Version UI steps (the API cannot write those field configurations without orphaning existing assignments).
